...
use foiacquire::repository::migration::ProgressCallback;
use foiacquire::repository::pool::SqlitePool;
use foiacquire::repository::util::{is_postgres_url, redact_url_password, validate_database_url};
use foiacquire::repository::{DatabaseExporter, DatabaseImporter, SqliteMigrator};
...
async fn copy_with_postgres(
    ...
    use foiacquire::repository::migration_postgres::PostgresMigrator;
...
async fn run_analyze_if_needed(
    target: &foiacquire::repository::migration_postgres::PostgresMigrator,
    options: &CopyOptions,
) -> anyhow::Result<()> {
    if !options.analyze {
        return Ok(());
