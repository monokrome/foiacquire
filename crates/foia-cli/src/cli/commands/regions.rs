...
pub async fn cmd_load_regions(settings: &Settings, file: Option<&str>) -> anyhow::Result<()> {
    ...
    let is_postgis = foia::with_conn_split!(doc_repo.pool,
        sqlite: _conn => Ok::<bool, diesel::result::Error>(false),
        postgres: conn => {
            let result: Result<Vec<foia::repository::diesel_document::DocIdRow>, _> =
    ...
    foia::with_conn_split!(doc_repo.pool,
    ...
    let country_count =
        load_geojson_features(&doc_repo, foia::gis_data::COUNTRIES, "country").await?;
    println!(
    ...
    let state_count =
        load_geojson_features(&doc_repo, foia::gis_data::STATES_PROVINCES, "state").await?;
    println!(
...
async fn load_geojson_features(
    ...
    for feature in features {
        ...
        let result = foia::with_conn_split!(doc_repo.pool,
            sqlite: _conn => Ok::<_, diesel::result::Error>(()),
            postgres: conn => {
