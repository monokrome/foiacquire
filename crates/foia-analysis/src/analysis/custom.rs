...
impl CustomBackend {
    ...
    fn apply_privacy_env(&self, cmd: &mut Command) {
        ...
        if let Some(proxy) = foia::privacy::socks_proxy_from_env() {
            cmd.env("ALL_PROXY", proxy);
        }

        // Forward FOIA_DIRECT if set
