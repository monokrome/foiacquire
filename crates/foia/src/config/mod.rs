//! Configuration management for foia using the prefer crate.
...
pub const DEFAULT_DATABASE_FILENAME: &str = "foia.db";
...
pub struct Config {
    ...
    /// Accepts a plain filename (e.g. "foia.db") which is joined with data_dir,
...
impl ResolvedData {
    ...
    /// - If path is a directory, look for foia.db inside
...
impl Config {
    ...
    /// Automatically discovers foia config files in standard locations.
    pub async fn load() -> Self {
        // Use prefer for file discovery, then parse with serde
        match prefer::load("foia").await {
            Ok(pref_config) => {
