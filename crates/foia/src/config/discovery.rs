...
//!
//! These structs define configuration for external discovery sources
//! (search engines, sitemaps, Wayback Machine, term extraction).
//!
//! Moved from `discovery/config.rs` to break circular dependencies
//! for workspace split (config is in core, discovery is a domain crate).
