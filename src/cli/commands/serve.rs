...
use crate::privacy::{CTorHiddenService, HiddenServiceProvider};
...
pub async fn cmd_serve(
    ...
    if !hs_config.is_enabled() {
        ...
        return crate::server::serve(settings, &host, port).await;
    }
...
async fn start_with_ctor(
    settings: &Settings,
    _config: &Config,
    hs_config: &crate::privacy::HiddenServiceConfig,
    host: &str,
    port: u16,
) -> anyhow::Result<()> {
    ...
    let result = crate::server::serve(settings, host, port).await;
...
async fn start_with_arti(
    settings: &Settings,
    _config: &Config,
    _hs_config: &crate::privacy::HiddenServiceConfig,
    host: &str,
    port: u16,
) -> anyhow::Result<()> {
