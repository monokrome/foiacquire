...
pub async fn cmd_load_regions(settings: &Settings, file: Option<&str>) -> anyhow::Result<()> {
    ...
    let is_postgis = crate::with_conn_split!(doc_repo.pool,
        sqlite: _conn => Ok::<bool, diesel::result::Error>(false),
        postgres: conn => {
            let result: Result<Vec<crate::repository::diesel_document::DocIdRow>, _> =
    ...
    crate::with_conn_split!(doc_repo.pool,
    ...
    let countries_data = include_str!("../../../data/ne_110m_admin_0_countries.geojson");
    let country_count = load_geojson_features(&doc_repo, countries_data, "country").await?;
    println!(
    ...
    let states_data = include_str!("../../../data/ne_110m_admin_1_states_provinces.geojson");
    let state_count = load_geojson_features(&doc_repo, states_data, "state").await?;
    println!(
...
async fn load_geojson_features(
    doc_repo: &crate::repository::DieselDocumentRepository,
    geojson_str: &str,
    region_type: &str,
) -> anyhow::Result<usize> {
    ...
    for feature in features {
        ...
        let result = crate::with_conn_split!(doc_repo.pool,
            sqlite: _conn => Ok::<_, diesel::result::Error>(()),
            postgres: conn => {
                use diesel_async::SimpleAsyncConnection;
